...
-->	  
	...
	검색하고자 하는 회원 ID : <input id="inputId" name="">
	<input type="button" id="btn3" value="조회"></p>
	<!-- submit 으로 해도 됨 button 으로 하면 밑에 자바스크립트로 해주면 됨 -->
	<div id="output3"></div>
			...
			$.ajax({
				...
				success : function(result){
					if( result.status == "fail" ){
						...
						/* const mem = result.memeber 를 이런식으로 변수로 지정해서 써도 됨 ex) ID : mem.id*/
						$("#output3").html(value)
					}
	...
	<h3>4. 응답데이터로 조회된 여러 bean객체들이 담겨있는 ArrayList 받기</h3>
	<input type="button" id="btn4" value="클릭"> <br><br>
...
//				data : , 필요없으면 안써도 됨
				success : function(result){
					...
					for(let i=0; i<result.length; i++){
						...
						result1 += "<tr><td>";
						result1 += result[i].id + "</td><td>" + result[i].name + "</td><td>" + gen + "</td><td>" + result[i].email;						
						result1 += "</td></tr>";
					}
					$("tbody").html(result1);
				}
